//所属組織変更管理テーブルの行が追加されたとき、「申請状況」が"理事承認待ち"・"承認済・処理中"・"処理完了"のとき、当該行を編集不可にする。
(() => {
    "use strict";
    ...
    kintone.events.on(['app.record.create.change.所属組織変更管理', 'app.record.edit.change.所属組織変更管理'], function(event) {
        ...
        for (const row of rows) {
          if (row.value.申請状況.value === '承認済・処理中' || row.value.申請状況.value === '処理完了' || row.value.申請状況.value === '理事承認待ち' || row.value.申請状況.value === '棄却') {
            isApproved = true;
            break;
          }
        }
